...
def home():
    ...
    # ticket1 = Beginner(question="Как называется набор однотипных данных, имеющий общее для всех своих элементов имя?", var1="Множество", var2="Массив", var3="Запись", var4="Набор команд для компьютера", right_answer="Массив")
    # ticket2 = Beginner(question="Как называется процесс перестановки элементов массива с целью упорядочивания их в соответствии с каким-либо критерием?", var1="Поиск", var2="Перебор", var3="Сортировка", var4="Проверка", right_answer="Сортировка")
    # ticket3 = Beginner(question="Как называется алгоритм, который на каком-либо шаге обращается сам к себе?", var1="Рекурсивным", var2="Циклическим", var3="Вспомогательным", var4="Обновляемый",
    #                    right_answer="Рекурсивным")
    # ticket4 = Beginner(question="Что называется наследованием?", var1="Это механизм, посредством которого производный класс получает элементы родительского и может дополнять либо изменять их свойства и методы", var2="Это механизм переопределения методов базового класса", var3="Это механизм, посредством которого производный класс получает все поля базового класса", var4="Это механизм, посредством которого производный класс получает элементы родительского, может их дополнить, но не может переопределить",
    #                    right_answer="Это механизм, посредством которого производный класс получает элементы родительского и может дополнять либо изменять их свойства и методы")
    # ticket5 = Beginner(question="Переменная - это", var1="Неизвестная величина", var2="Именованная  область памяти", var3="Название одной ячейки памяти",
    #                    var4="Выражение которое постоянно меняется", right_answer="Именованная  область памяти")
    # ticket6 = Beginner(question="Какой тип циклов существует?", var1="Цикл типа “если”", var2="Цикл типа “пока”", var3="Цикл типа “иначе”", var4="Цикл типа “выбор”",
    #                    right_answer="Цикл типа “пока”")
    # ticket7 = Beginner(question="Какое название носят формальные языки, предназначенные для записи компьютерных программ?", var1="Языки программирования", var2="Язык транскрибирования", var3="Языки передачи", var4="Языки записи",
    #                    right_answer="Языки программирования")
    # ticket8 = Beginner(question="Какой из пунктов относится к принципа ООП ?", var1="Инкапсуляция, верификация, наследование", var2="Трансполяция, верефикация, периодизация", var3="Наследование, инкапсуляция, полиморфизм",
    #                    var4="Обобщение, полиморфизм, трансполяция", right_answer="Наследование, инкапсуляция, полиморфизм")
    # ticket9 = Beginner(question="В C++ для доступа к элементам объекта используются:", var1="При обращении через имя объекта – точка, при обращении через указатель – операция «-˃»", var2="При обращении через имя объекта – два двоеточия, при обращении через указатель – операция «точка»", var3="При обращении через имя объекта – точка, при обращении через указатель – два двоеточия", var4="При обращении через имя объекта – два двоеточия, при обращении через указатель – операция «-˃»",
    #                    right_answer="При обращении через имя объекта – точка, при обращении через указатель – операция «-˃»")
    # ticket10 = Beginner(question="Полиморфизм – это :", var1="Средство, позволяющее использовать одно имя для обозначения действий, общих для родственных классов", var2="Средство, позволяющее в одном классе использовать методы с одинаковыми именами", var3="Средство, позволяющее в одном классе использовать методы с разными именами для выполнения одинаковых действий", var4="Средство, позволяющее перегружать функции для работы с разными типами или разным количеством аргументов",
    #                    right_answer="Средство, позволяющее использовать одно имя для обозначения действий, общих для родственных классов")
...
# Отслеживание страницы "Добавление нового вопроса 1/4"
...
def vvod_vopr_t1():
    ...
    if request.method == "POST":
        ...
        if (int(type) != 1 and int(type) != 2):
            return "Неправильно введён номер теста"

        if (right_ans != ans1) and (right_ans != ans2) and (right_ans != ans3) and (right_ans != ans4):
            return "Ошибка, правильный ответ не совпадает ни с одним из вариантов ответа"
...
# Отслеживание страницы "Вопросы для обновления 1/4"
...
def obnov_vopr_t1():
    ...
    if request.method == "POST":
        ...
        if (password == PASSWORD):
            ...
            if (int(type) != 1 or int(type) == 2):
                return "Неправильно введен номер теста"
        else:
            return "У вас нет прав для внесения изменений в БД"
    else:
        return render_template('obnov_vopr_test1.html')

# Отслеживание страницы "Добавление нового вопрос (ввод ответа)"
...
# Отслеживание страницы "Вопросы для обновления (тест - ввод ответа)"
